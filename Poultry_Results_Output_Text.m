...
lb(1)=min(mle_outbreak_poultry_farm_County(~no_farms));
ub(1)=max(mle_outbreak_poultry_farm_County(~no_farms));
md(1)=median(mle_outbreak_poultry_farm_County(~no_farms));
...
cc(1)=sum(mle_outbreak_risk_poultry_farm_County>risk_t);
...
cc(1)=prctile(mle_potential_outbreak_poultry_farm_County(~no_farms),95);
...
temp=mle_potential_outbreak_poultry_farm_County>prctile(mle_potential_outbreak_poultry_farm_County(~no_farms),95);
cc(1)=mean(mle_outbreak_risk_poultry_farm_County(temp)<risk_t);

cc(2:3)=per_risk_under_10_95;
...
% State outbreaks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[~,IndxS]=sort(mle_outbreak_poultry_farm_State,'descend');

for ii=1:3
    cc(1)=mle_outbreak_poultry_farm_State(IndxS(ii));

    cc(2:3)=outbreak_poultry_farm_State_95(IndxS(ii),:);
...
[~,IndxS]=sort(mle_spillover_risk_poultry_farm_State,'descend');
for ii=1:3
    fprintf(['State Rank ' num2str(ii) ' for spillover-risk: '  State_Name{IndxS(ii)} ' (' num2str(mle_spillover_risk_poultry_farm_State(IndxS(ii)),'%3.2f') ': ' num2str(spillover_risk_poultry_farm_State_95(IndxS(ii),1),'%3.2f') char(8211) num2str(spillover_risk_poultry_farm_State_95(IndxS(ii),2),'%3.2f') ') \n']);
end
 fprintf('\n');
