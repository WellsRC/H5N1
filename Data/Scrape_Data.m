...
POPULATION_SIZE_2010=NaN.*zeros(height(US_County),1);
% 
for cc=1:height(US_County)
    t_find=strcmpi(County_ID,US_County.GEOID{cc});
    if(sum(t_find)>0)
        POPULATION_SIZE_2010(cc,1)=Data.Total__Estimate__TotalPopulation(t_find);
...
% 
POPULATION_SIZE_2020=NaN.*zeros(height(US_County),1);
% 
for cc=1:height(US_County)
    t_find=strcmpi(County_ID,US_County.GEOID{cc});
    if(sum(t_find)>0)
        POPULATION_SIZE_2020(cc,1)=Data.Estimate__Total__TotalPopulation(t_find);
%         X_temp(cc,2)=Under_15(t_find);
%         X_temp(cc,3)=Age_15_64(t_find);
%         X_temp(cc,4)=Over_64(t_find);
    end
...
% 
X=[POPULATION_SIZE_2010 POPULATION_SIZE_2020];
Y=X_temp(:,1);
t_nan=~isnan(sum(X,2)) & ~isnan(Y);
t_need=~isnan(sum(X,2)) & isnan(Y);
mdl_fit=fitlm(X(t_nan,:),Y(t_nan));
