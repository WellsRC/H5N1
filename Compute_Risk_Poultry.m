...
overall_risk_poultry_farm_State=zeros(length(State_Name),length(par_est));
exposure_risk_poultry_farm_State=zeros(length(State_Name),length(par_est));
susceptible_risk_poultry_farm_State=zeros(length(State_Name),length(par_est));
...
for mm=1:length(w_AIC)
    ...
    overall_risk_poultry_farm_County(County_Farms==0,mm)=NaN;
    exposure_risk_poultry_farm_County(County_Farms==0,mm)=NaN;
    susceptible_risk_poultry_farm_County(County_Farms==0,mm)=NaN;
    spillover_risk_poultry_farm_County(County_Farms==0,mm)=NaN;

    for ss=1:length(State_Name)
        t_state=strcmp(State_Name{ss},US_County.STATE_NAME);

        c_r=overall_risk_poultry_farm_County(t_state,mm);
...
avg_overall_risk_poultry_farm_State=overall_risk_poultry_farm_State*w_AIC;
avg_exposure_risk_poultry_farm_State=exposure_risk_poultry_farm_State*w_AIC;
avg_susceptible_risk_poultry_farm_State=susceptible_risk_poultry_farm_State*w_AIC;
avg_spillover_risk_poultry_farm_State=spillover_risk_poultry_farm_State*w_AIC;
