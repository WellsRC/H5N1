...
load('Dairy_Risk_AIC.mat','mle_post_outbreak_dairy_farm_State');


[Outbreak_State,R_Indx]=sort(Outbreak_State,'descend');
mle_post_outbreak_dairy_farm_State=mle_post_outbreak_dairy_farm_State(R_Indx,:);
...
for pp=1:4
    ...
    for ii=1:nr(pp)
        for jj=1:3
            Outbreak_Post_State=squeeze(mle_post_outbreak_dairy_farm_State(jj+3.*(ii-1)+12.*(pp-1),:));
