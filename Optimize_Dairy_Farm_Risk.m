...
lb=[-6 -18.*ones(1,size(X_County,1))  -4 -4];
ub=[ 0 -2.*ones(1,size(X_County,1)) -1  -1];
...
lb=[-7 -12.*ones(1,size(X_County,1))  -6 -6];
ub=[ 3   ones(1,size(X_County,1))   1  1];
...
ub=[ 50  4.*ones(1,size(X_County,1))   2  2];
...
if(~isempty(x0))
    ...
    ub=[ 50  5.*ones(1,size(X_County,1))   3    3];
