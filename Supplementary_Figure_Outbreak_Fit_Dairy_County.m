...
for pp=1:4
    ...
    for ii=1:nr(pp)
        for jj=1:3
            ...
            subplot('Position',[0.075+0.32.*(jj-1) 0.77-0.24.*(ii-1) 0.28 0.2]);
            ...
            if(Upper_add_County(jj+3.*(ii-1)+12.*(pp-1))>0)
                for cc=1:length(AfC)
                    patch(cc+[-0.25 -0.25 0.25 0.25],[AfC(cc) AfC_upper(cc) AfC_upper(cc) AfC(cc)],hex2rgb('#F0810F'),'FaceAlpha',0.33,'linestyle','none')
                end
            else
            ...
            mxl=ceil(max(max(AfC_upper),max(post_outbreak_County(:))).*1.01);
            dxtick=1;
            if(mxl>=10)
                mxl=ceil(mxl./10).*10;
                if(mxl<20)
