function Compute_Risk_Dairy_Reduced_Connectivity(red_con)
...
for mm=1:height(Dairy_Model)
    [X_County,Y_County,County_Farms,~,~,~,~,~,~,Dairy_Network,logic_par] = Dairy_Covariates(Dairy_Model.Model_H5N1{mm},Dairy_Model.Model_Farm{mm},Dairy_Model.Model_Stratified_Operations{mm});
    x=par_est{mm};
    ...
    if(length(beta_x)>1)
        ...
        if(logic_par(4))
            for ss=1:height(US_County)
                t_state=strcmp(US_County.STATE_NAME{ss},US_County.STATE_NAME);
                Dairy_Network(ss,~t_state)=(1-red_con).*Dairy_Network(ss,~t_state);
            end
        end
    end
    ...
    for ss=1:length(State_Name)
        ...
        c_r=c_r(t_inc);
        overall_risk_dairy_farm_State(ss,mm)=1-prod((1-c_r));

        c_r=exposure_risk_dairy_farm_County(t_state,mm);
        w_c=US_County.TOTAL_DAIRY_OPERATIONS(t_state);
        t_inc=w_c>0 & ~isnan(c_r);
        c_r=c_r(t_inc);
        exposure_risk_dairy_farm_State(ss,mm)=1-prod((1-c_r));
